package deferred
{
	 ...
	 */   
	public class DeferredMovieClip extends DeferredImage implements IAnimatable
	{
		...
		/** Creates a movie clip from the provided textures and with the specified default framerate.
		 *  The movie will have the size of the first frame. */  
		public function DeferredMovieClip(textures:Vector.<Texture>, fps:Number=12,_depth:Number=1,_emmissive:Number=0,_translucent:Boolean=false)
		{
			convertTexToDefTex(textures);
			
			if (textures.length > 0)
			{
				super((textures[0] as DeferredTexture),_depth,_emmissive,_translucent);
				init(textures, fps);
			}
			else
			{
		...
		private function convertTexToDefTex(textures:Vector.<Texture>):void{
			for(var i:int = 0;i<textures.length;i++){
				textures[i] = new DeferredTexture(textures[i]);
			}
		}
